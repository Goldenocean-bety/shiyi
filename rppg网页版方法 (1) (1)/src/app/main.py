...
from src.vitals.heart_rate_tscan import HeartRateMonitorTSCAN
    ...
    try:
        ...
        while True:
            ...
            if frame_idx % 10 == 0:
                ...
                # Get PPG signal if available
                ppg_data = []
                if 'raw_signal' in result and result['raw_signal'] is not None:
                    ppg_data = result['raw_signal'].tolist()
                    # Normalize PPG signal for better visualization
                    if len(ppg_data) > 0:
                        ppg_data = [(x - min(ppg_data)) / (max(ppg_data) - min(ppg_data)) if max(ppg_data) != min(ppg_data) else x for x in ppg_data]
...
@app.websocket("/ws/process_tscan")
async def websocket_process_tscan(websocket: WebSocket):
    """WebSocket endpoint for real-time video processing with TS-CAN."""
    await websocket.accept()
    
    try:
        # Check if PyTorch is available
        if not TORCH_AVAILABLE:
            await websocket.send_json({"error": "PyTorch not available for TS-CAN"})
            return
        
        # Wait for video path
        data = await websocket.receive_json()
        video_path = data.get("video_path")
        use_lite = data.get("use_lite", True)
        ...
        await websocket.send_json({
            ...
            "fps": fps,
            "total_frames": total_frames,
            "model": "TS-CAN Lite" if use_lite else "TS-CAN"
        })
        ...
        while True:
            ...
            # Send update every 5 frames (TS-CAN is faster)
            if frame_idx % 5 == 0:
                # Encode frame as base64
                _, buffer = cv2.imencode('.jpg', result['frame_annotated'], 
                                         [cv2.IMWRITE_JPEG_QUALITY, 50])
                ...
                await websocket.send_json({
                    ...
                    "frame": frame_b64,
                    "ppg_data": ppg_data,
                    "method": result.get('method', 'tscan')
                })
